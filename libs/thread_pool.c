...
// task in one go
static int get_task(thread_pool* w, get_task_result* res)
{
  int done;
  closure_node* c = NULL;
  res->task = NULL;
...
int add_task(thread_pool* w, void (*func)(void*), void* cl)
{
...
static int join_all(pthread_t* t, size_t num)
{
  assert(t != NULL || num == 0);
  int ret = 0;
  if (t) {
...
static int cancel_all(pthread_t* t, size_t num)
{
  assert(t != NULL || num == 0);
  int ret = 0;
  if (t) {
...
static int cancel_and_join_all(pthread_t* t, size_t num)
{
...
static void* worker_thread(void* cl)
{
  ...
  while (1) {
    ...
    // If there's thread_pool, then execute the thread_pool regardless of done.
    if (res.task != NULL) {
...
static int thread_pool_common(struct thread_pool_s* w, size_t num_threads)
{
...
int stop_thread_pool(struct thread_pool_s* w)
{
  ...
  if (thread_pool_not_spinning(w)) {
    log_errorln("Trying to stop spinning a thread pool that's not spinning");
    return 1;
  }
